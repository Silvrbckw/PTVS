...
for VS_VERSION in (15, 16, 17):
    print(f'Writing to {THEME_PKGDEF.format(VS_VERSION)}')
    ...
    for src in WindowsPath(__file__).parent.glob("*.vstheme"):
        ...
        data = [struct.pack('I', len(categories))]
        for category in categories:
            ...
            for color in colors:
                ...
                data.extend((struct.pack('I', len(namebytes)), namebytes))
                ...
                if bg_e is None:
                    ...
                    data.extend(
                        (
                            struct.pack('b', CT_VALUES[bg_e.get('Type')]),
                            struct.pack('I', int(bg_e.get('Source'), 16)),
                        )
                    )
                ...
                if fg_e is None:
                    ...
                    data.extend(
                        (
                            struct.pack('b', CT_VALUES[fg_e.get('Type')]),
                            struct.pack('I', int(fg_e.get('Source'), 16)),
                        )
                    )
        payload = b''.join(data)
